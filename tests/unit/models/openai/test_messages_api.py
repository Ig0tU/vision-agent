    ...
    messages = [
        MessageParam(
            ...
            content=[
                ...
                ToolUseBlockParam(
                    id="call_1", name="get_weather", input={"location": "Berlin"}
                ),
            ],
        )
    ]
...
def test_transform_tool_result_message(messages_api: OpenAiMessagesApi) -> None:
    messages = [
        MessageParam(
            role="user",
            content=[ToolResultBlockParam(tool_use_id="call_1", content="It's sunny.")],
        )
    ]
    transformed = messages_api._transform_messages(messages)
    assert transformed == [
...
def test_model_get(model: OpenAiModel, mock_openai_client: MagicMock) -> None:
    ...
    result = model.get(
        "What is in the image?", source, response_schema=None, model="gpt-4o"
    )
